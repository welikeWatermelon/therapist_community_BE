...
- [ ] **V25 마이그레이션 운영 영향 기록** (PR #55)
  - `ALTER TABLE ... UPDATE ... CREATE INDEX` 실행 시 전체 row 락 발생. 현재 데이터 규모에선 문제없지만, `docs/ops/BRANCH_OPERATIONS_GUIDE.md`나 deploy 전달 노트에 "V25 적용 시 짧은 쓰기 락" 언급 필요.
  - **다시 볼 트리거**: 다음 [`CLOUD_DELIVERY_PACKAGE.md`](docs/ops/CLOUD_DELIVERY_PACKAGE.md) 업데이트 시 또는 테이블 row 수가 100k를 넘을 때.
