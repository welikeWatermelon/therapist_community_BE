...
```
Tier 1: 공식 운영 규칙 (tracked)
  docs/ops/*, AGENTS.md, CLAUDE.md
   ↓
Tier 2: 글로벌 개인 운영 문서 (untracked)
  ~/.codex/AGENTS.md, ~/.claude/CLAUDE.md
   ↓
Tier 3: 메인 세션 기록 (untracked)
  backend-now/harness/main-session.md, anti-patterns.md
   ↓
Tier 4: 작업 세션 기록 (untracked)
...
작업 세션 발견(Tier 4) → 메인 세션 흡수(Tier 3) → 공식화(Tier 1). 승격 시 하위에서 제거 또는 redirect 한 줄만 남긴다(중복 방지). 자세한 승격 룰은 §11.12 참조.
...
- **`PostCompact`** (Claude Code): compact 이후 Claude가 만든 summary를 `checkpoint_post-compact_<timestamp>.md`에 저장
