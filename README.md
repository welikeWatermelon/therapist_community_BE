...
- **[#14] HNSW 인덱스를 만들었는데 플래너가 쓰지 않는 문제**
...
- **[#11] 3072차원 임베딩은 pgvector 인덱스를 걸 수 없는 문제**
...
- **[#10] 스케줄러와 이벤트 리스너가 같은 job을 중복 처리하는 문제**
...
- **[#9] 자기호출 시 @Transactional이 무시되는 AOP 함정**
...
프로젝트 전반에서 반복되는 시그니처 패턴입니다.

| 패턴 | 적용처 |
|---|---|
| `AFTER_COMMIT` + `@Async` 이벤트로 본 트랜잭션과 부수효과 분리 | 알림, 분석, 임베딩, AI 댓글, 지식 인제스션 |
| `FOR UPDATE SKIP LOCKED` + `PESSIMISTIC_WRITE`로 폴러/이벤트 중복 방지 | AI 댓글 job, 지식문서, 집계 커서 |
| `@Lazy` self-injection / 별도 빈으로 AOP 자기호출 우회 | AI 댓글, 지식 인제스션, 임베딩 실패 기록 |
| 1분 → 5분 → 30분 지수 백오프 재시도 | AI 댓글, 지식 인제스션 |
| DELETE → INSERT 멱등 집계 + progress 커서 | 시간별/일별 분석 집계 |
| DDD 경계 — 크로스 도메인 접근은 상대 도메인 Service 경유 | 전 도메인 (ADR-002/003으로 위반 17건 정리 완료) |
| partial index로 핫패스만 인덱싱 | 피드, 구인공고, 메시지함 |
| soft delete (`deletedAt`) | 게시글, 댓글, 메시지, 구인공고 |
...
Flyway 마이그레이션 V1~V52로 스키마 전체 이력을 코드로 관리합니다.

| 영역 | 주요 테이블 |
|---|---|
| 회원/인증 | `users`, `refresh_tokens`(token_family), `therapist_verifications` |
| 게시글 | `therapy_posts`(search_text, content_embedding, popularity_score), `post_images`, `post_attachments`, `therapy_post_videos`, `therapy_post_downloads` |
| 소셜 | `therapy_post_comments`, `post_reactions`, `comment_reactions`, `therapy_post_scraps`, `follows`, `messages` |
| 알림 | `notifications` |
| AI | `knowledge_documents`, `knowledge_chunks`(vector 3072), `post_ai_comment_jobs` |
| 분석 | `user_events`(월별 파티션), `post_hourly_stats`, `therapist_expertise_daily`, `aggregation_progress` |
| 운영 | `job_posts`, `feature_flags` |
...
프로파일: `local`(로컬) / `dev`(스테이징) / `prod`(운영) / `perf`(성능 측정 — SQL 로깅 + 응답시간 히스토그램). AI 기능(검색 임베딩, 자동댓글)은 기본 비활성이며 API 키 설정 후 프로퍼티/Feature Flag로 켤 수 있습니다.
