...
5. 작업 생명주기 관리: 새로운 작업을 시작할 때나, 사용자가 **작업 종료·PR 생성·머지를 명확히 지시한 때**(예: "PR 만들어", "이거 머지해", "작업 마무리해", "푸시해줘") [docs/ops/AGENT_WORKFLOW.md](docs/ops/AGENT_WORKFLOW.md) §3·§4 절차를 따른다. "좋아", "완료", "OK" 같은 단순 긍정은 리뷰·설명·답변에 대한 동의일 수 있으므로 자동 커밋/PR 흐름의 트리거로 해석하지 않는다 — 모호하면 사용자에게 "지금 PR 흐름으로 가도 됩니까?"를 짧게 확인. 기존 `harness/Workflow.md` *(local-only)*는 본 문서로 마이그레이션됨 (사용자 워크트리에서 한 줄 redirect로 정리 또는 삭제).
