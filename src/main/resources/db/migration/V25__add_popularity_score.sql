...
-- 2. 기존 데이터 초기값 세팅
--    공식: reactions * 3 + scraps * 2 + (created_at epoch초 / 86400)
UPDATE therapy_posts p
SET popularity_score = (
    COALESCE((SELECT COUNT(*) FROM therapy_post_reactions r WHERE r.post_id = p.id), 0) * 3
    + COALESCE((SELECT COUNT(*) FROM therapy_post_scraps s WHERE s.post_id = p.id), 0) * 2
